...
# obs_id to low letters
# obs_name OK
head(unique(as.factor(dams$obs_name)))
# datasource OK
# country OK
# obs_height round(num)
# obs_presence_eel_pass NA 0 1 to NA no yes
unique(as.factor(dams$obs_presence_eel_pass))
# fishway_type code to type
# unique(dams$fishway_type[dams$obs_presence_eel_pass =="1"])
# [1] <NA> 271  277  275  270 
# Levels: 270 271 273 275 277 279
# basin to low letters

summary(dams)

dams$fishway_type <- as.factor(dams$fishway_type)
dams$obs_presence_eel_pass<- as.factor(dams$obs_presence_eel_pass)


head(dams[is.na(dams$fishway_type) & dams$obs_presence_eel_pass == "1",])

dams$fishway_type_2 <- dams$fishway_type
dams$fishway_type_2[is.na(dams$fishway_type) & dams$obs_presence_eel_pass == "1"] <- "279"

unique(as.factor(dams$fishway_type_2))
...
damssf_reproj$obs_name[is.na(damssf_reproj$obs_name)] <- "NA"
...
#levels(as.factor(as.character(dams$obs_presence_eel_pass)))
#[1] "0" "1"


# Convert uppercase letters to lowercase
damssf_reproj$basin <- sapply(damssf_reproj$basin, tolower)
# Maria : small and big ones have eel pass.

# Fish way type from code to name
# unique(as.factor(dams$fishway_type))
# [1] <NA> 279  273  271  277  275  270 
# Levels: 270 271 273 275 277 279
damssf_reproj$fishway_type_name[damssf_reproj$fishway_type_2=="270"] <-"Vertical slot"
damssf_reproj$fishway_type_name[damssf_reproj$fishway_type_2=="271"] <-"Pool type"
damssf_reproj$fishway_type_name[damssf_reproj$fishway_type_2=="273"] <-"Denil pass"
damssf_reproj$fishway_type_name[damssf_reproj$fishway_type_2=="275"] <-"Rock ramp"
damssf_reproj$fishway_type_name[damssf_reproj$fishway_type_2=="277"] <-"Lateral canal"
damssf_reproj$fishway_type_name[damssf_reproj$fishway_type_2=="279"] <-"Unknown"

damssf_reproj$fishway_type_name<-addNA(damssf_reproj$fishway_type_name)
...
popuplist = paste0( "Obstacle name: "
                    ...
                    , "<br>"
                    , "Fish pass: "
                    , damssf_reproj$fishway_type_name
                    , "<br>"
                    , "Fish pass available for eels: "
                    , damssf_reproj$obs_presence_eel_pass
                    , "<br>"
                    , "Data source: "
...
damssf_reproj$obs_presence_eel_pass<- as.factor(damssf_reproj$obs_presence_eel_pass)
...
damssf_reproj$type[damssf_reproj$obs_height > 10 & is.na(damssf_reproj$obs_presence_eel_pass)]<-"dam"
damssf_reproj$type[damssf_reproj$obs_height > 0 & damssf_reproj$obs_height < 10 & is.na(damssf_reproj$obs_presence_eel_pass)] <- "smalldam"
...
## 2019_06_13
dams.spain$obs_actual_stand <- 0
dams.spain$obs_actual_stand <- "yes"

dams.spain$obs_presence_eel_pass<- as.character(dams.spain$obs_presence_eel_pass)
dams.spain$obs_presence_eel_pass<- factor(dams.spain$obs_presence_eel_pass)

dams.spain$obs_eel_pass <- 0
dams.spain$obs_eel_pass[is.na(dams.spain$obs_presence_eel_pass)] <- "NA"
dams.spain$obs_eel_pass[dams.spain$obs_presence_eel_pass == "0"] <- "no"
dams.spain$obs_eel_pass[dams.spain$obs_presence_eel_pass == "1"] <- "yes"

dams.spain$obs_eel_pass <-as.factor(dams.spain$obs_eel_pass)
...
dams.spain$type<-revalue(dams.spain$type, c("dam"="Dam > 10m", "greendam"="Dam > 10m with fish pass available for eels",
                           "greensmall" = "Dam < 10m with fish pass available for eels", "smalldam" = "Dam < 10m"))
...
## USER AUTHETICATION 
## https://cultureofinsight.shinyapps.io/shinyauthr/
## https://github.com/treysp/shiny_password
## https://github.com/yanirmor/shiny-user-management/blob/master/server.R
